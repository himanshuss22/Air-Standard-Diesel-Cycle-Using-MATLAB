...
%---------Atmospheric Inputs------------
P_0=100;                  %Atmospheric Pressure                                     (kPa)
T_0=60;                   %Combustion Chamber Temperature at Start of Compression
...
%----------Engine Inputs------
V=input('Import Engine Displacement Volume(Litr): ');
N=input('Import Number of Cylinders: ');
...
disp('---------------CxHy Fuel Composition------------ ');
x=input('Please import number of Carbon atoms: ');
y=input('Please import number of Hydrogen atoms: ');
b=x/y;
AF_s=(34.56*(b+4))/(12.011+(1.008*b));                        %Stoichiometric Air/Fuel ratio (Heywood (5-3))
Z = ['For Complete Combustion Assumption The Air-Fuel Mass Ratio Must be ',num2str(AF_s),' or Higher'];
disp(Z)
AF=input('Import Air-Fuel Mass Ratio: ');
...
T_3=((FHV*eta_c)/((AF+1)*C_p))+T_2;     %Temprature of Gas Mixture in Cylinder at State 3 (Deg K)
...
V_6=V_c;                                %Volume of Gas Mixture in Cylinder at State 6     (Cubic Meter)
...
OPD=W_bdat/V;                               %Outpot per Displacement                              (KW/Litr)
